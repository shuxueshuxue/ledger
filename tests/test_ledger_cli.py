...
class LedgerCliTests(unittest.TestCase):
    ...
    def test_init_renders_agents_from_packaged_ledger_agent_instructions(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Repo Structure Taste", agents)
            self.assertIn("one concept, one home, one source of truth", agents)
            self.assertIn("shadow source-of-truth trees", agents)
            self.assertIn("workflow residue in shipping paths", agents)
            self.assertIn("duplicate top-level or near-top-level trees", agents)
